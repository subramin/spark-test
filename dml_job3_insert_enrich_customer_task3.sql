...
-----------------------------------------------------------------------------------------------------------
delete from DATA where NAME='CUSTOMER3_ENRICH';
insert into DATA(NAME,TANSFORM_TYPE,TRANSFORM_INPUTS,FITER_FORMULA) 
values('CUSTOMER3_ENRICH','ENRICH','CUSTOMER1_RAW','AND(GREATER_THAN(COL("CUSTOMER_ID"),100),LESS_THAN(COL("CUSTOMER_ID"),200))');

delete from DATA_COLUMN where DATA_NAME='CUSTOMER3_ENRICH';
insert into DATA_COLUMN(DATA_NAME,NAME,SEQ,DATA_TYPE,ENRICH_FORMULA) values('CUSTOMER3_ENRICH','CUSTOMER_ID',1,'I','COL("CUSTOMER_ID")');
insert into DATA_COLUMN(DATA_NAME,NAME,SEQ,DATA_TYPE,ENRICH_FORMULA) values('CUSTOMER3_ENRICH','FIRST',2,'S','COL("FIRST")');
insert into DATA_COLUMN(DATA_NAME,NAME,SEQ,DATA_TYPE,ENRICH_FORMULA) values('CUSTOMER3_ENRICH','LAST',3,'S','COL("LAST")');
insert into DATA_COLUMN(DATA_NAME,NAME,SEQ,DATA_TYPE,ENRICH_FORMULA) values('CUSTOMER3_ENRICH','ADDRESS',4,'S','COL("ADDRESS")');
insert into DATA_COLUMN(DATA_NAME,NAME,SEQ,DATA_TYPE,ENRICH_FORMULA) values('CUSTOMER3_ENRICH','ZIPCODE',5,'S','COL("ZIPCODE")');
insert into DATA_COLUMN(DATA_NAME,NAME,SEQ,DATA_TYPE,ENRICH_FORMULA) values('CUSTOMER3_ENRICH','CUST_TYPE',6,'S','COL("CUST_TYPE")');
insert into DATA_COLUMN(DATA_NAME,NAME,SEQ,DATA_TYPE,ENRICH_FORMULA) values('CUSTOMER3_ENRICH','FULL_NAME',7,'S','CONCAT(COL("LAST"),", ",COL("FIRST"))');
insert into DATA_COLUMN(DATA_NAME,NAME,SEQ,DATA_TYPE,ENRICH_FORMULA) values('CUSTOMER3_ENRICH','CITY',8,'S','LOOKUP("LOOKUP_ZIPCODE_CITY",COL("ZIPCODE"))');
insert into DATA_COLUMN(DATA_NAME,NAME,SEQ,DATA_TYPE,ENRICH_FORMULA) values('CUSTOMER3_ENRICH','STATE',9,'S','LOOKUP("LOOKUP_ZIPCODE_STATE",COL("ZIPCODE"))');
-----------------------------------------------------------------------------------------------------------
...
-----------------------------------------------------------------------------------------------------------
delete from TASK where NAME='Task3_Ingest_CUSTOMER3_ENRICH';
insert into TASK(NAME,TASK_TYPE) values('Task3_Ingest_CUSTOMER3_ENRICH','DATA_TRANFORM_ENRICH');
...
insert into WORKFLOW_TASK(WF_ID,TASK_ID,TASK_SEQ) 
	...
	(select ID from TASK where NAME='Task3_Ingest_CUSTOMER3_ENRICH') as TASK_ID,
	3 as TASK_SEQ;
-----------------------------------------------------------------------------------------------------------
...
insert into IO_INFO(STORAGE_TYPE,FILE_PATH,DATA_NAME) values('CSV','/tmp/data/curated/customer_ingested_dq.csv','CUSTOMER1_RAW');
--insert into IO_INFO(STORAGE_TYPE,FILE_PATH) values('AVRO','/tmp/data/curated/customer_ingested_dq.avro');
-----------------------------------------------------------------------------------------------------------
delete from IO_INFO where STORAGE_TYPE='CSV' and FILE_PATH='/tmp/data/gold/customer_enriched.csv';
insert into IO_INFO(STORAGE_TYPE,FILE_PATH,DATA_NAME) values('CSV','/tmp/data/gold/customer_enriched.csv','CUSTOMER3_ENRICH');
-----------------------------------------------------------------------------------------------------------
...
delete from TASK_IO where 1=1 
	and TASK_ID=(select ID from TASK where NAME='Task3_Ingest_CUSTOMER3_ENRICH') 
	and IO_ID=(select ID from IO_INFO where STORAGE_TYPE='CSV' and FILE_PATH='/tmp/data/curated/customer_ingested_dq.csv' and DATA_NAME='CUSTOMER1_RAW')
	and IO_SEQ=1
	and IO_TYPE='INPUT';
insert into TASK_IO(TASK_ID,IO_ID,IO_SEQ,IO_TYPE) select  
	(select ID from TASK where NAME='Task3_Ingest_CUSTOMER3_ENRICH') as TASK_ID,
	(select ID from IO_INFO where STORAGE_TYPE='CSV' and FILE_PATH='/tmp/data/curated/customer_ingested_dq.csv' and DATA_NAME='CUSTOMER1_RAW') as IO_ID,
	1 as IO_SEQ,
	'INPUT' as IO_TYPE;
-----------------------------------------------------------------------------------------------------------	
delete from TASK_IO where 1=1 
	and TASK_ID=(select ID from TASK where NAME='Task3_Ingest_CUSTOMER3_ENRICH') 
	and IO_ID=(select ID from IO_INFO where STORAGE_TYPE='CSV' and FILE_PATH='/tmp/data/gold/customer_enriched.csv')
	and IO_SEQ=2
	and IO_TYPE='OUTPUT';
insert into TASK_IO(TASK_ID,IO_ID,IO_SEQ,IO_TYPE) select  
	(select ID from TASK where NAME='Task3_Ingest_CUSTOMER3_ENRICH') as TASK_ID,
	(select ID from IO_INFO where STORAGE_TYPE='CSV' and FILE_PATH='/tmp/data/gold/customer_enriched.csv') as IO_ID,
	2 as IO_SEQ,
	'OUTPUT' as IO_TYPE;
-----------------------------------------------------------------------------------------------------------	
delete from TASK_IO where 1=1 
...
insert into TASK_IO(TASK_ID,IO_ID,IO_SEQ,IO_TYPE) select  
	...
	'LOOKUP_INPUT' as IO_TYPE;
-----------------------------------------------------------------------------------------------------------
delete from JOB where JOB_UID='JOB_BE17AA764B194FCAAC2E679B9653F26A_3';
insert into JOB(JOB_UID,WF_ID,TASK_ID,TASK_SEQ,SCHEDULE_TIME,LAST_UPD_TIME,STATUS)
select 'JOB_BE17AA764B194FCAAC2E679B9653F26A_3' as JOB_UID, 
